...
const getSpecificAllBlogs = async function (req, res) {
  try {
      ...
      //     ...data
      // }
      if(Object.keys(data).length==0) return res.status(400).send("query data not found")
      //let blogData = await blogModel.find(filter)//.populate("authorId")
      let blogData = await blogModel.find({$and:[data,{isDeleted:false},{isPublished:true}]})
      if(!blogData) return res.status(404).send({status:false, msg:"No such blog exists"})
